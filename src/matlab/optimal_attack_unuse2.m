...
function [ret] = optimal_attack_1(u,k,R,ru,rpc,xu)
...
for i=1:72 % we have 72 rp
    ...
    for j=1:(35-k) % find the (35-k) minimal pairs for two given rss vector
        ...
        while x <= size(ru2,2) && y <= size(ri2,2) %after while loop, will find 1 pair
            if ru2(x) ==1
                x=x+1;continue
            end
            if ri2(y) ==1
            ...
            if abs(ru2(x) - ri2(y)) < min % update the minimal pair
                min = abs(ru2(x) - ri2(y));
                minx = x; miny=y;
            end
            if ru2(x) < ri2(y)
        ...
        ru2(minx)=(1);ri2(miny)=(1); %delete the used pair!
