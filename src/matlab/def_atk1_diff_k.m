...
%     for i=1:10:360  
%         tmp=rss;
%         fprintf("atk1-t3-d2: User indxe: %d, FakeAp Num: %d\n",i,k);
%         ru = U(i,:); 
...
% end
% 
% universe_a1t3_minmax2=universe;
% avg_a1t3_minmax2=avg;
%save avg_a1t3_dist_minmax2.mat universe_a1t3_minmax2 avg_a1t3_minmax2;
...
%     for i=1:10:360  
%         tmp=rss;
%         fprintf("atk1-t3-d4: User indxe: %d, FakeAp Num: %d\n",i,k);
%         ru = U(i,:); 
%         xu = upc(i,:); % current user rss and coordinates
%         
%         %res(i)=optimal_attack_knn(i,k,tmp,ru,rpc,xu);
...
% end
% 
% universe_a1t3_minmax4=universe;
% avg_a1t3_minmax4=avg;
%save avg_a1t3_dist_minmax4.mat universe_a1t3_minmax4 avg_a1t3_minmax4;

%% lamda 0:15 k=2,4,6...
% k means number of fake aps
% lamda means number of dropped aps
...
for xx = 1:15
    ...
    for i=1:10:360  
        tmp=rss;
        fprintf("atk1-t3-k4: User indxe: %d, FakeAp Num: %d\n",i,k);
        ru = U(i,:); 
        xu = upc(i,:); % current user rss and coordinates
        ...
        % first change to worst case
        %ru(fid(end-k+1:end))=ri(fid(end-k+1:end));
        ru(fid(end-k:end))=ri(fid(end-k:end));
        
        % second: drop strategy:
        % drop best 8 and worst 8

        drop_ap=fid(end-xx+2:end);
        drop_ap=[drop_ap,fid(1:xx-1)];
        ru(drop_ap) = []; % randomly generate K fake RSS btw
        tmp(:,drop_ap) = [];
        idx = rnd_cnk(ru,tmp);
        res(i) = pdist2(rpc(idx,:),xu);
    end
    avg_k4(xx)= mean(res(1:10:360));
    %universe(:,k)=res; % k layer
end
...
for xx = 1:15
    ...
    for i=1:10:360  
        tmp=rss;
        fprintf("atk1-t3-k8: User indxe: %d, FakeAp Num: %d\n",i,k);
        ru = U(i,:); 
        xu = upc(i,:); % current user rss and coordinates
...
%     for i=1:10:360  
%         tmp=rss;
%         fprintf("atk1-t3-k16: User indxe: %d, FakeAp Num: %d\n",i,k);
%         ru = U(i,:); 
%         xu = upc(i,:); % current user rss and coordinates
%         
%         %res(i)=optimal_attack_knn(i,k,tmp,ru,rpc,xu);
...
%         res(i) = pdist2(rpc(idx,:),xu);
%     end
%     avg_k16(xx)= mean(res(1:10:360));
%     %universe(:,k)=res; % k layer
% end

%% set the figure properties

% plot(avg_k4,'-b*','MarkerSize',6,'LineWidth',1)
% hold on
save ./mat/lambda_k.mat avg_k4 avg_k8 avg_k16;

% x=0:34;
% plot(x,avg_k4,'-ro','MarkerSize',8,'LineWidth',2)
% hold on
% plot(x,avg_k8,'-b+','MarkerSize',8,'LineWidth',2)
% %axis([0 14 0 26]) %same as xlim & ylim
% grid on
% xlim([0,14])
% % ylim([0 25])
% xlabel('\lambda')
% ylabel('Average error distance (m)')
% legend('\it k=4','\it k=8')
