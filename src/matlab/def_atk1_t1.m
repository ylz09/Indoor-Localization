...
for k = 1:2:25
    ...
    for i=1:10:360  
        tmp=rss;
        fprintf("atk1-t1-d2: User indxe: %d, FakeAp Num: %d\n",i,k);
        ru = U(i,:); 
...
universe_a1t1_minmax2=universe;
avg_a1t1_minmax2=avg;
save avg_a1t1_dist_minmax2.mat universe_a1t1_minmax2 avg_a1t1_minmax2;
...
for k = 1:2:25
    ...
    for i=1:10:360  
        tmp=rss;
        fprintf("atk1-t1-d4: User indxe: %d, FakeAp Num: %d\n",i,k);
        ru = U(i,:); 
        xu = upc(i,:); % current user rss and coordinates
...
universe_a1t1_minmax4=universe;
avg_a1t1_minmax4=avg;
save avg_a1t1_dist_minmax4.mat universe_a1t1_minmax4 avg_a1t1_minmax4;
...
for k = 1:2:25
    ...
    for i=1:10:360  
        tmp=rss;
        fprintf("atk1-t1-d8: User indxe: %d, FakeAp Num: %d\n",i,k);
        ru = U(i,:); 
        xu = upc(i,:); % current user rss and coordinates
        
        %res(i)=optimal_attack_knn(i,k,tmp,ru,rpc,xu);
        %[fid,imax]=def_atk1_type3(i,k,mv,rss,ru,rpc,xu);
        [fid,imax]=optimal_attack_knn(i,k,tmp,ru,rpc,xu); % knn is not the right name, it's just version of otpimal attack
        ri=rss(imax,:);
        % knn
        %res(i)=knn(fake_rss,tmp,rpc,xu);
...
universe_a1t1_minmax8=universe;
avg_a1t1_minmax8=avg;
save avg_a1t1_dist_minmax8.mat universe_a1t1_minmax8 avg_a1t1_minmax8;
