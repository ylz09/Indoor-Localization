...
for p=1:35
    ...
    for j=1:k
        ...
        i=iids(j);
        xu=upc(u,:);
        klog(j) = target_dist2(p-1,u,i,umap,rmap,xu,rpc);
    end
    dist2=[dist2;klog];
end
...
for p=1:35
    ...
    for j=1:k
        ...
        i=iids(j);
        xu=upc(u,:);
        klog(j)=target_dist3(p-1,u,i,umap,rmap,mydb,xu,rpc);
    end
    dist3=[dist3;klog];
end
...
save ./mat/target_k_cdf.mat dist1 dist2 dist3 dist4;
target_cdf_fig
%% 2 problem
...
function [feasible] = target_dist2(k,u,i,umap,rmap,xu,rpc)
ru=umap(u,:);
ri=rmap(i,:);
diff = abs(ru-ri);
[rdff,idx]=sort(diff);
...
function [feasible] = target_dist3(k,u,i,umap,rmap,mydb,xu,rpc)
ru=umap(u,:);
fri=mydb(i,:);
diff = abs(ru-fri);
[rdff,idx]=sort(diff);
