...
%save ./mat/avg_a1t3_dist_minmax4.mat universe_a1t3_minmax4 avg_a1t3_minmax4;
...
for k = 1:2:25
    ...
    for i=1:10:360  
        tmp=rss;
        fprintf("atk1-t3-d8: User indxe: %d, FakeAp Num: %d\n",i,k);
        ru = U(i,:); 
        xu = upc(i,:); % current user rss and coordinates
        
        %res(i)=optimal_attack_knn(i,k,tmp,ru,rpc,xu);
        [fid,imax]=def_atk1_t3(i,k-1,mv,fdb,ru,rpc,xu);
        ri=fdb(imax,:);
        % knn
        %res(i)=knn(fake_rss,tmp,rpc,xu);
...
universe_a1t3_minmax8=universe;
avg_a1t3_minmax8=avg;
%save ./mat/avg_a1t3_dist_minmax8.mat universe_a1t3_minmax8 avg_a1t3_minmax8;
