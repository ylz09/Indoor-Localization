...
universe5=zeros(360,2,35);
avg5=zeros(1,35);

for k = 1:35
    res=zeros(360,2);     
    for i=1:360        
        fprintf("atk5: User indxe: %d, FakeAp Num: %d\n",i,k);
        ru = U(i,:); xu = upc(i,:); % current user rss and coordinates
        res(i,:)=random_attack(i,k,rss,ru,rpc,xu);
    end
    avg5(k)= mean(res(:,2));
    universe5(:,:,k)=res; % k layer
end
save ./mat/avg_rnd_dist.mat universe5 avg5;
